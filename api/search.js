...
ANALIZZA QUESTA DESCRIZIONE: "${payload.productDescription}"
...
OUTPUT: Solo i numeri delle classi pertinenti separati da virgole (es: 25 oppure 9,42)`;
        ...
        try {
            ...
            console.log('Risposta AI per classificazione:', identifiedClassesString);
            
            // Parse e validazione delle classi
            const identifiedClasses = identifiedClassesString
                .replace(/[^\d,]/g, '') // Rimuovi tutto tranne numeri e virgole
                .split(',')
                .map(c => parseInt(c.trim()))
                .filter(c => !isNaN(c) && c >= 1 && c <= 45)
                .filter((v, i, a) => a.indexOf(v) === i); // Rimuovi duplicati

            console.log('Classi estratte e validate:', identifiedClasses);

            if (identifiedClasses.length === 0) {
                // Fallback: analisi basata su keywords
                console.log('Nessuna classe identificata dall\'AI, uso fallback...');
                const fallbackClasses = identifyClassesByKeywords(payload.productDescription);
                if (fallbackClasses.length > 0) {
                    console.log('Classi identificate con fallback:', fallbackClasses);
                    return fallbackClasses;
                }
                throw new Error("Impossibile identificare classi di Nizza pertinenti.");
            }
            
            console.log('Classi finali identificate:', identifiedClasses);
            return identifiedClasses;
            
        } catch (error) {
            console.error('Errore nella classificazione AI:', error);
            // Usa il fallback basato su keywords
            const fallbackClasses = identifyClassesByKeywords(payload.productDescription);
            if (fallbackClasses.length > 0) {
                return fallbackClasses;
            }
            throw new Error("Errore nell'identificazione delle classi di Nizza");
        }
    }

    // Nuova funzione di fallback per identificare classi tramite keywords
    function identifyClassesByKeywords(description) {
        const descLower = description.toLowerCase();
        const classes = new Set();
        
        // Mappatura keywords -> classe
        const keywordMap = {
            3: ['cosmetici', 'profumi', 'trucco', 'shampoo', 'sapone', 'crema', 'deodorante'],
            9: ['software', 'app', 'applicazione', 'computer', 'hardware', 'programma'],
            16: ['carta', 'quaderni', 'penne', 'matite', 'libri', 'stampati'],
            18: ['borse', 'valigie', 'zaini', 'portafogli', 'pelletteria'],
            25: ['scarpe', 'abbigliamento', 'vestiti', 'magliette', 'pantaloni', 'cappelli'],
            30: ['caffè', 'tè', 'pasta', 'pane', 'dolci', 'cioccolato'],
            35: ['pubblicità', 'marketing', 'vendita', 'commercio', 'consulenza aziendale'],
            41: ['formazione', 'educazione', 'corsi', 'eventi', 'intrattenimento'],
            42: ['sviluppo software', 'programmazione', 'consulenza informatica'],
            43: ['ristorante', 'bar', 'hotel', 'catering', 'albergo'],
            44: ['medico', 'bellezza', 'parrucchiere', 'estetista', 'spa']
        };
    ...
    // Chiamata alla funzione di classificazione
    const identifiedClasses = await classifyProducts();
